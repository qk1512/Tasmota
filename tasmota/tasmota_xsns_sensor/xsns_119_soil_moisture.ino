/*
  xsns_119_soilmoisture.ino - Soil Moisture Sensor using Modbus support for Tasmota
...
#define GPIO_SM_RX 13
#define GPIO_SM_TX 14

#include <TasmotaModbus.h>
TasmotaModbus *SMModbus;
...
void SMInit(void)
{
    ...
    {
        SMModbus = new TasmotaModbus(Pin(GPIO_RS485_RX), Pin(GPIO_RS485_TX));
        uint8_t result = SMModbus -> Begin();
        if(result)
        {
            if(result == 2)
            {
        ...
        {
            AddLog(LOG_LEVEL_INFO, PSTR("Soil Moisture Modbus init failed %d"),result);
        }
    }
    else
    {
        AddLog(LOG_LEVEL_INFO, PSTR("Soil Moisture no GPIOs assigned"));
    }
}
...
void SMreadData(void)
{
    ...
    if (!requestSent)
    {
        SMModbus->Send(0x01, 0x03, modbusRequests[requestIndex].regAddr, modbusRequests[requestIndex].regCount);
        lastRequestTime = millis();
        requestSent = true;
    }

    if (requestSent && (millis() - lastRequestTime >= 200))
    {
        if (SMModbus->ReceiveReady())
        {
            uint8_t buffer[10]; // Max expected response size
            uint8_t error = SMModbus->ReceiveBuffer(buffer, sizeof(buffer));
